...
Promise.all([pg(config), phantomInit.createPhantomSession(app)]).then(async values => {
  ...
  console.log('Verificare și inițializare bază de date...');
  await dbInit(app.locals.db);
